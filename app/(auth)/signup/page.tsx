...
function SignupContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // If user confirmed immediately (email confirm disabled in Supabase settings)
    if (data.session) {
  ...
  if (success) {
    return (
      <div className="grid min-h-screen place-items-center bg-bg px-4">
        <div className="w-full max-w-sm rounded-card border border-line bg-card p-8 text-center shadow-card-lg">
          <p className="text-[36px]">📬</p>
          <h1 className="mt-3 text-lg font-bold text-fg">Check your email</h1>
          <p className="mt-2 text-[13px] text-fg-sub">
            We sent a confirmation link to <span className="text-fg">{email}</span>. Click it to activate your account.
          </p>
          <Link href="/login" className="mt-5 block text-[13px] font-medium text-accent hover:underline">
            Back to sign in →
          </Link>
        </div>
      </div>
