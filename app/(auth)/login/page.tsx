    ...
    <div className="grid min-h-screen place-items-center bg-bg px-4 py-12">
      <div className="w-full max-w-sm rounded-card border border-line bg-card p-8 shadow-card-lg">
        ...
        <form onSubmit={handleEmail} className="space-y-3">
          ...
          {unconfirmed && (
            <div className="rounded-btn border border-warning/30 bg-warning/10 px-3 py-2.5 text-[12px]">
              <p className="font-medium text-warning">
                Email belum dikonfirmasi. Silakan cek inbox Anda.
              </p>
              <div className="mt-1.5">
                {resendStatus === "sent" ? (
                  <span className="text-success">✓ Email konfirmasi telah dikirim ulang.</span>
                ) : (
                  <button
                    type="button"
                    onClick={handleResend}
                    disabled={resendStatus === "sending"}
                    className="text-accent underline underline-offset-2 hover:opacity-80 disabled:opacity-60"
                  >
                    {resendStatus === "sending" ? "Mengirim…" : "Kirim ulang email konfirmasi"}
                  </button>
                )}
              </div>
            </div>
